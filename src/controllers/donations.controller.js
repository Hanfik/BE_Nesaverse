...
const getDonations = async (req, res, next) => {
  try {
    const rows = await sql`SELECT * FROM donations ORDER BY created_at DESC LIMIT 50`;
    res.json(rows);
  } catch (err) { next(err); }
};
...
const createDonation = async (req, res, next) => {
  try {
    ...
    const rows = await sql`INSERT INTO donations (name,is_anonymous,amount,message) VALUES (${displayName},${is_anonymous||false},${amount},${message||null}) RETURNING *`;
